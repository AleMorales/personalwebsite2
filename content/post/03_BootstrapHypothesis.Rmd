...
In this article I will show alternative methods to test differences in means. These methods are known as *bootstrapping* methods and they come in many flavours. Here I will focus on the *parametric bootstrap* and *non-parametric bootstrap*. In the final section, the result from these methods will be compared to the t applied to the original and log scale to illustrate the impact that these choices can have on the results.
...
By sampling from each population we can compute the sampling distribution of the difference of sample means:

```{r, cache = TRUE}
N = 5e4
sample_mean_C = future_map_dbl(1:N, ~ mean(popC$rand(18)))
sample_mean_D = future_map_dbl(1:N, ~ mean(popD$rand(18)))
sample_mean_HT = future_map_dbl(1:N, ~ mean(popHT$rand(18)))
sample_mean_HTD = future_map_dbl(1:N, ~ mean(popHTD$rand(18)))
```
...
```{r, cache = TRUE}
N = 5e4
boot_sample_mean_C_D = future_map_dbl(1:N, ~ mean(sample(boot_mean_Biomass_C_D, 18,replace = TRUE)))
boot_sample_mean_C_HT = future_map_dbl(1:N, ~ mean(sample(boot_mean_Biomass_C_HT, 18,replace = TRUE)))
boot_sample_mean_C_HTD = future_map_dbl(1:N, ~ mean(sample(boot_mean_Biomass_C_HTD, 18,replace = TRUE)))
boot_sample_mean_D = future_map_dbl(1:N, ~ mean(sample(boot_mean_Biomass_D, 18,replace = TRUE)))
boot_sample_mean_HT = future_map_dbl(1:N, ~ mean(sample(boot_mean_Biomass_HT, 18,replace = TRUE)))
boot_sample_mean_HTD = future_map_dbl(1:N, ~ mean(sample(boot_mean_Biomass_HTD, 18,replace = TRUE)))
```
